...
class OrderController extends BaseController
{
    ...
    public function get(){
        ...
        $order = order_get_item(array('uid'=>$this->uid, 'order_id'=>$order_id));
        if ($order) {
            $order['create_time'] = date('Y-m-d H:i:s', $order['create_time']);
            $item = order_get_goods(array('order_id'=>$order_id));
    ...
    public function pay(){
        ...
        if ($pay_type == 1){
            ...
            if (wallet_cost($this->uid, $order['total_fee'])){
                order_update_item(array('order_id'=>$order_id), array('pay_status'=>1, 'pay_type'=>1, 'pay_time'=>time()));
