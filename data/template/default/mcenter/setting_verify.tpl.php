...
<div class="userinfo-div">
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="formtable">
    	<tr>
        	<td width="64">性别</td>
            <td>
            <?php if(is_array($lang[sex_items])) { foreach($lang[sex_items] as $k => $v) { ?>            	<input type="radio" value="$k" name="userinfo[usersex]"<?php if($k==$userinfo[usersex]) { ?> checked="checked"<?php } if($userinfo[locked]) { ?> disabled="disabled"<?php } ?>> $v
            <?php } } ?>            </td>
            <td width="40">生日</td>
